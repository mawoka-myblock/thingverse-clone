...
from helpers.security.auth import get_current_user, get_password_hash, get_user_from_id, verify_password, \
    create_access_token, get_user_from_mail, get_user_from_username, authenticate_user, optional_current_user
...
async def create_thing(background_tasks: BackgroundTasks, data: BaseInputThing,
    ...
    if await col("files").find_one({"file_id": data.file}) is None:
    ...
    thing_dict.update({"creation_date": creation_date, "id": thing_id, "user_id": current_user.id, "hash": thing_hash})
...
async def get_thing(thing_id: str):
    thing = await col("things").find_one({"id": thing_id})
    if thing is None:
...
async def like_thing(thing_id: str, current_user: UserInDB = Depends(get_current_user)):
    thing = await col("things").find_one({"id": thing_id})
    if thing is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Thing does not exist")
   await col("likes")"""